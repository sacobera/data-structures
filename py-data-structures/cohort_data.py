...
def all_houses(filename):
    ...
    cohort_data = open(filename) #opens the filename

    for line in cohort_data: #iterate over the data in the file
      house = line.rstrip().split('|')[2] #remove extra characters from the side of the string, as well as split every data
        if house: 
            houses.add(house) #add to the lines of house every iteration 
    return houses
